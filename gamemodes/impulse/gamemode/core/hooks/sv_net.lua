...
util.AddNetworkString("HatchetCitizenTerminalOpen")
...
util.AddNetworkString("impulseLegalStorageOpen")
...
net.Receive("impulseATMWithdraw", function(len, ply)
	...
	if ply:CanAffordBank(amount) then
		...
		ply:EmitSound("ambient/machines/keyboard7_clicks_enter.wav", 50, 95, 0.5, CHAN_AUTO)
...
net.Receive("impulseInvDoMove", function(len, ply)
	...
	if ply.currentStorage:GetClass() == "impulse_storage_public" then
		...
		if item.Illegal then
			return ply:Notify("You may not access or store illegal items in here.")
		end
	end
	
	if ply.currentStorage:GetClass() == "impulse_storage" then
		local item = impulse.Inventory.Items[impulse.Inventory.ClassToNetID(item.class)]
		...
		if not item.Illegal then
			return ply:Notify("You may not access or store legal items in here.")
		end
	end
...
net.Receive("impulseDoConfiscate", function(len, ply)
	...
	if item:GetPos():DistToSqr(ply:GetPos()) < (200 ^ 2) then
		...
		ply:Say("/me confiscates a "..itemName..".")
		item:Remove()
