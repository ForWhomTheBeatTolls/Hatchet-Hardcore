...
function impulse.SetupPlayer(ply, dbData)
	...
	ply:SetSyncVar(SYNC_RPVORTNAME, dbData.vortrpname, true)
	ply:SetSyncVar(SYNC_RPCITY, dbData.city, true)
...
function GM:PlayerDeath(ply, killer)
	...
	for v,k in pairs(inv) do
		...
		if item.DropOnDeath then
			...
			local physObjWeight = physObj:GetMass()
			ent:SetCollisionGroup(COLLISION_GROUP_WEAPON)
			physObj:SetDamping(15, 15)
			--print("1, 1:"..physObj:GetMass())
			physObj:SetMass(1)
			--print("1, 2:"..physObj:GetMass())
			--print("Started!!!")
			timer.Simple(0.1, function()
			physObj:SetDamping(1, 1)
	...
	for v,k in pairs(allPlayers) do
		...
		if k:Alive() then
			if k.nextHungerUpdate < ctime then
				...
				local hungerdmg = DamageInfo()
				hungerdmg:SetDamage(1)
				hungerdmg:SetDamageType(DMG_NERVEGAS)
				
				
				if k:GetSyncVar(SYNC_HUNGER, 100) < 1 then
					if k:Health() > 0 then
						k:TakeDamageInfo(hungerdmg)
						k:EmitSound("vo/npc/male01/moan0"..math.random(1,5)..".wav", 50)
					end
					k.nextHungerUpdate = ctime + 15
				else
			...
			if (k.nextHungerHeal or 0) < ctime then
				...
				if hunger >= 80 and k:Health() < 90 then
					k:SetHealth(math.Clamp(k:Health() + 1, 0, 90))
