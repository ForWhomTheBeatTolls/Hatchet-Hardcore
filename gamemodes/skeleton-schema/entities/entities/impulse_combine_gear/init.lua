...
function ENT:Use(ply)
	...
	if ( ply:Team() == TEAM_OTA ) and ( ply:GetTeamClass() == 3 ) then
	...
	if ( ply:Team() == TEAM_OTA ) and ( ply:GetTeamClass() == 2 ) then
		ply:Notify("You are already an Overwatch Soldier. Giving gear instead.")
	if ( table.HasValue( MACEwl, plyid64 ) ) and !ply:HasInventoryItem("wep_shotgun") then
		--ply:GiveInventoryItem(table.SortByMember(APEXgear, "IMP" ))
		for _, v in ipairs(MACEgear) do
			ply:GiveInventoryItem(v, 1, true, true)
		end
		end
		end
	if ( ply:Team() == TEAM_OTA ) and ( ply:GetTeamClass() == 1 ) then
		ply:Notify("You are already an Overwatch Soldier. Giving gear instead.")
	if ( table.HasValue( ECHOwl, plyid64 ) ) and !ply:HasInventoryItem("wep_smg") then
		--ply:GiveInventoryItem(table.SortByMember(APEXgear, "IMP" ))
		for _, v in ipairs(ECHOgear) do
			ply:GiveInventoryItem(v, 1, true, true)
		end
		end
		end
