...
function PANEL:Init()
	...
	self.suitType:SetText("Normal Suit")
	self.suitType:AddChoice("Blue Suit")
	self.suitType:AddChoice("Normal Suit")
	if not (displayBodygroup == 6 or displayBodygroup == 5) then
		self.suitType:SetEnabled(false)
	end
	self.suit = 1
	self.suitType.OnSelect = function(_,i)
		self.suit = i-1
		self.modelPreview.Entity:SetBodygroup(1,5+self.suit)
	end
