...
function EYEVIEW_Think()
	if CLIENT then
		...
		if EYEVIEW_THINK_DELAY < CurTime() then
			if LocalPlayer():IsPlayer() and LocalPlayer():LookupBone("ValveBiped.Bip01_Head1") then if eyeview_enabled ~= false then LocalPlayer():SetBodygroup(5, 0) end end
...
if CLIENT then
	function EYEVIEW_CalcView(ply, origin, angles, fov, near, far)
		...
		if ply:Alive() and ply:GetActiveWeapon():IsValid() then
			if ply:IsValid() and ply:Alive() and ply:GetMoveType() ~= MOVETYPE_NOCLIP and (ply:GetActiveWeapon():GetClass() ~= "gmod_tool") and (IsValid(impulse.MainMenu) and not impulse.MainMenu:IsVisible()) and (ply:GetActiveWeapon():GetClass() ~= "weapon_physgun") and ply:Ping() < 240 then
				---------------------###CHECKS START###---------------------
				local boneindex
				local boneindex2
				local headcheck = {}
				headcheck.start = LocalPlayer():GetPos()
				if ply:Team() == TEAM_VORTIGAUNT then
					boneindex = LocalPlayer():LookupBone("ValveBiped.head") or 1
					boneindex2 = LocalPlayer():GetBonePosition(boneindex) or LocalPlayer():GetPos()
					headcheck.start = boneindex2
				else
					boneindex = LocalPlayer():LookupBone("ValveBiped.Bip01_Head1") or 1
					boneindex2 = LocalPlayer():GetBonePosition(boneindex) or LocalPlayer():GetPos()
					headcheck.start = boneindex2
				end
				headcheck.endpos = headcheck.start + Vector(0, 0, 5)
				--headcheck.endpos = headcheck.start - Vector(0,0,5)
				headcheck.filter = function(ent) return ent:GetClass() == "prop_physics" end
				...
				local headcheck2 = {}
				if ply:Team() == TEAM_VORTIGAUNT then
					headcheck2.start = LocalPlayer():GetBonePosition(boneindex)
				else
					headcheck2.start = LocalPlayer():GetBonePosition(boneindex)
				end
				headcheck2.endpos = headcheck2.start - Vector(0, 0, 25)
				--headcheck.endpos = headcheck.start - Vector(0,0,5)
				headcheck2.filter = function(ent) return ent:GetClass() == "prop_physics" end
				...
				local clipcheck = {}
				if ply:Team() == TEAM_VORTIGAUNT then
					clipcheck.start = LocalPlayer():GetBonePosition(boneindex) + Vector(1, 0, -6)
					clipcheck.endpos = LocalPlayer():GetBonePosition(boneindex) + Vector(1, 0, -1)
				else
					clipcheck.start = LocalPlayer():GetBonePosition(boneindex) + Vector(1, 0, -6)
					clipcheck.endpos = LocalPlayer():GetBonePosition(boneindex) + Vector(1, 0, -1)
				end
				clipcheck.filter = function(ent) if ent:GetClass() == "player" then return false end end
				...
				if LocalPlayer():Team() == TEAM_VORTIGAUNT then
					eyeview.fov = impulse.GetSetting("eyeview_fov") * 1.2
				else
				...
				if impulse.GetSetting("view_thirdperson") then
					...
					t.filter = function(ent)
						...
						if ent.GetNoDraw(ent) then return false end
						return true
					end

					local tr = util.TraceLine(t)
					pos = tr.HitPos
					if tr.Fraction < 1.0 then pos = pos + tr.HitNormal * 5 end
					local fov = impulse.GetSetting("view_thirdperson_fov")
					local wep = LocalPlayer():GetActiveWeapon()
					if wep and IsValid(wep) and wep.GetIronsights and not wep.NoThirdpersonIronsights then fov = Lerp(FrameTime() * 15, wep.FOVMultiplier, wep:GetIronsights() and wep.IronsightsFOV or 1) * fov end
	...
	function EYEVIEW_ShouldDrawLocalPlayer(ply)
		if ply:GetActiveWeapon():IsValid() then if ply:IsValid() and ply:Alive() and ply:GetMoveType() ~= MOVETYPE_NOCLIP and (ply:GetActiveWeapon():GetClass() ~= "gmod_tool") and (IsValid(impulse.MainMenu) and not impulse.MainMenu:IsVisible()) and (ply:GetActiveWeapon():GetClass() ~= "weapon_physgun") and ply:Ping() < 240 then return true end end
	end
