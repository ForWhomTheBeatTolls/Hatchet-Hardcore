...
net.Receive("impulseHL2RPCameraAlert", function()
	...
	elseif crime == CRIME_WEAPON then
		col = 4
	elseif crime == CRIME_CURFEW then
		col = 3
	...
	impulse.AddCombineWaypoint("CAMERA SPOTTED " ..CRIME_NICENAMES[crime]..".", pos, duration, 7, col, 900, nil, nil, citizen)
...
net.Receive("impulseHL2RPCameraBroken", function()
	...
	LocalPlayer():SendCombineMessage("CAMERA DOWNED", Color(150, 154, 158))
	impulse.AddCombineWaypoint("CAMERA DOWNED", camera:GetPos(), 100, 7, 5, 900)
end)
